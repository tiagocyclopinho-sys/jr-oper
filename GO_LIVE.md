...
## 📍 ONDE PARAMOS — 22/08/2026, ~01h

**Feito:**
- Migrações **22, 23 e 24 aplicadas** no Supabase. Não precisa rodar SQL de novo.
- Código corrigido até a build **`sync-4.7.9`** (cache `v4.7.9`), ainda **não publicado**.
- Sincronização provada de ponta a ponta: uma devolução foi aberta no PC,
  apareceu no celular, recebeu Retorno Físico lá e voltou atualizada. O caminho
  principal funciona.

**Pendente — retomar por aqui:**

1. **Publicar** na Vercel (PASSO 3)
2. **`Ctrl + Shift + R`** em cada aparelho e conferir `buildSync: "sync-4.7.9"`
   no console via `jrDiagnosticoSync()` (PASSO 4)
3. Aguardar ~40s e conferir os motoristas:
   ```sql
   SELECT count(*) AS total, count(*) FILTER (WHERE cnh <> '') AS com_cnh FROM motoristas;
   ```
   Esperado **`total = 41`, `com_cnh = 2`** (39 da planilha + 2 de teste).
   Se ainda vier `2 / 2`, o aparelho não está na build nova — volte ao item 2.
4. Remover os 2 de teste (bloco comentado no fim da `migration_24_unique_parcial.sql`)
5. **Reset Global** — só depois que TODOS os aparelhos estiverem em `sync-4.7.9`
6. Teste de aceite (PASSO 6)

**Estado do banco nesta hora:** `motoristas` 2 (os dois de teste),
`ocorrencias_devolucao` 0, `controle_viagens` 15, `veiculos` 89, `ajudantes` 38,
`usuarios` 8. O `2 / 2` em motoristas é esperado — as migrações abriram a porta
no banco, mas quem envia os 39 é o aparelho, e ele ainda está com a build antiga.
